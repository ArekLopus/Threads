...
public class Shop3Find1CFwithCustomExecutor {
	...
	public Shop3Find1CFwithCustomExecutor() throws InterruptedException, ExecutionException {
		
		long start = System.nanoTime();
		
		CompletableFuture<?>[] cfa = findPricesStream("myPhone")	//Tu tworzymy TYLKO Stream, trzeba to jeszcze uruchomić!
			.map( fc -> fc.thenAccept(s -> System.out.println(s + " (done in " + ((System.nanoTime() - start) / 1_000_000) + " msecs)")))
			.toArray( size -> new CompletableFuture[size] );
		
		CompletableFuture.anyOf(cfa).join();
	...
	public static void main(String[] args) throws Exception {
		new Shop3Find1CFwithCustomExecutor();
	}
