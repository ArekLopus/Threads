...
public class ForkJoinPoolAsyncMode {
	...
	public static void main(String[] args) {
        ...
        ForkJoinPool pool = new ForkJoinPool(4, ForkJoinPool.defaultForkJoinWorkerThreadFactory, null, false);
    ...
    /**
     * A {@code ForkJoinTask} that prints a range if the range is smaller than a certain threshold; otherwise halves the range and proceeds recursively.
    ...
    private static class RecursiveRangeAction extends RecursiveAction {
        ...
        protected void compute() {
            if (end - start < 10) {
                ...
                System.out.format("%1$s [%2$d-%3$d] -< [%3$d-%4$d], fork async task %5$d%n", Thread.currentThread().getName(), start, mid, end, id);

                // Fork off ADDITIONAL asynchronous task that is never joined.
                ForkJoinTask.adapt(() -> {
                    System.out.format("%s additional async task %d done%n", Thread.currentThread().getName(), id);
                }).fork();
